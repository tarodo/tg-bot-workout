...
async def test_show_main_menu_new():
    ...
    # Mock update and context
    update = MagicMock()
    update.callback_query = None  # Explicitly set to None to avoid callback handling
...
@pytest.mark.asyncio
async def test_show_main_menu_start():
    """Test showing main menu with /start command."""
    # Mock update and context
    update = MagicMock()
    update.callback_query = None  # Explicitly set to None to avoid callback handling
    ...
    update.effective_user.full_name = "Test User"  # Add full_name
    update.effective_chat = MagicMock()
    update.effective_chat.send_message = AsyncMock()
    update.effective_chat.send_message.return_value = MagicMock(chat_id=123, message_id=456)
    ...
    with patch("bot.handlers.common.async_session", return_value=mock_session), patch(
        "bot.handlers.common.UserRepository", return_value=mock_user_repo
    ):
        await show_main_menu(update, context)
